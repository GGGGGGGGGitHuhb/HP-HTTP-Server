...
- formatter新增显式文件清单，提交hook仅检查暂存C++，保留索引/部分暂存/配置/路径/版本保护和不自动暂存；使用用户原样Google覆盖配置。独立Reviewer001 PASS：新基线与候选28/28、双smoke、0专项、五项ASan/UBSan、13项hook及两类负向反证通过。R1完成收口，仅本地提交、不推送，未合并/tag；R2未启动。
