...
int main(int argc, char* argv[]) {
    ...
    try {
        ...
        for (const auto option :
             {"--idle-timeout-ms", "--keep-alive-timeout-ms"}) {
            for (const auto value :
                 {"86400001", "-1", "+1", "", "1x", "184467440737095516160"}) {
                expect(
                    run_process(argv[1], {"--port", "0", "--root",
        ...
        expect(run_process(
                   argv[1],
                   {"--port", "0", "--root", fixture.root.string(),
                    "--shutdown-timeout-ms", "1", "--shutdown-timeout-ms", "0"})
                       .exit_code == 2,
        ...
        expect_contains(help.output, "defaults to 5000",
                        "help shutdown default");
