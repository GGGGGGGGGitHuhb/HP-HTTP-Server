...
inline std::vector<Rejection> Rejections() {
  ...
  return {{"G01-invalid-line", "GET  / HTTP/1.1\r\nHost: a\r\n\r\n", 400},
          ...
          {"G06-service-400",
           "GET /bad%20target HTTP/1.1\r\nHost: a\r\n\r\n",
           400,
           true},
          {"G05-total-16385", std::move(over), 400, false, true}};
}

inline const std::string kShortRequest = "GET / HTTP/1.1\r\nHost: a\r\n\r\n";

inline std::string PrefixState(std::size_t n) {
  if (kShortRequest[n - 1] == '\r') return "pending-CR";
  if (n < 16) return "request-line";
  if (n < kShortRequest.size() - 2) return "headers";
