...
void callback_lifetime_and_same_batch() {
  ...
  self =
      std::make_unique<Channel>(loop, first.observed.fd(), [&](std::uint32_t) {
        ++self_callbacks;
        self->remove();
        expect(
            !self->registered() && ::fcntl(first.observed.fd(), F_GETFD) >= 0,
...
void fd_reuse_and_failures() {
  ...
  EventLoop wrong;
  try {
    wrong.update_channel(good, EPOLLOUT);
