...
struct ConnectionTimeoutTestAccess {
    static ConnectionRegistry& registry(TcpServer& server, std::size_t index) {
        return server.worker_count_ ? *server.registries_.at(index) : *server.main_registry_;
    }
    ...
    static void expire(ConnectionRegistry& registry, int fd, TcpConnection::Identity identity) {
        registry.expire(fd, identity);
    }

    static bool stopping(TcpServer& server) {
        return server.stopping_.load();
    }

    static auto config(TcpServer& server) {
        return server.timeouts_;
    }
};
} // namespace hp::net
...
Snapshot snapshot(ServerHarness& harness, std::size_t workers, std::size_t index = 0) {
    ...
    require(result.wait_for(3s) == std::future_status::ready, "snapshot deadline");
    return result.get();
}
...
void idle_modes(const hp::http::StaticFileService& service) {
    for (std::size_t workers : {0U, 1U, 2U}) {
        ...
        require(after.connections == 0 && after.timers == 0, "pure timer immediately reclaimed");
...
void waiting_states(const hp::http::StaticFileService& service) {
    for (const auto config : {ConnectionTimeouts{0ms, 120ms}, ConnectionTimeouts{200ms, 0ms},
        ...
        require(initial.waiting == 0 && initial.timers == (config.idle.count() ? 1U : 0U),
        ...
        require(state.waiting == 1 && state.pending == 0, "pipeline drained before wait");
        require(state.timers == (config.idle.count() || config.keep_alive.count() ? 1U : 0U),
                "disabled policies allocate no timers");
        auto expected =
            config.idle.count() ? state.progress + config.idle.count() * 1000 : LLONG_MAX;
        if (config.keep_alive.count())
            expected = std::min(expected, state.wait + config.keep_alive.count() * 1000);
        if (state.timers)
        ...
        require(partial.progress > state.progress, "new bytes refresh idle and exit waiting");
        require(partial.timers == (config.idle.count() ? 1U : 0U), "partial has no keep timer");
        ...
        if (state.timers) {
            client.eof();
            require(ticks(Clock::now()) >= state.deadline, "combined deadline not early");
        } else {
            harness.server->request_stop();
        }
        harness.stop();
        std::cout << "waiting idle_ms=" << config.idle.count()
                  << " keep_ms=" << config.keep_alive.count() << " wait_us=" << state.wait
                  << " deadline_us=" << state.deadline << " partial_timers=" << partial.timers
...
void owner_progress_and_failures(const hp::http::StaticFileService& service) {
    ...
    require(Access::progress(connection) == initial && Access::timer(connection) == id,
            "EAGAIN and spurious writable do not refresh");
    connection.set_idle_wait(true);
    auto wait = Access::wait_since(connection);
    connection.set_idle_wait(true);
    require(Access::wait_since(connection) == wait, "duplicate wait does not renew");
    connection.set_idle_wait(false);
    const std::string partial = "GET /note.txt HTTP/1.1\r\nHost:";
    require(::send(pair.peer.fd(), partial.data(), partial.size(), MSG_NOSIGNAL) ==
                static_cast<ssize_t>(partial.size()),
            "partial send bytes");
    loop.poll_once(0);
    require(Access::progress(connection) > initial && !Access::wait_since(connection),
            "actual recv refresh");
    ...
    require(connection.state() == TcpConnection::State::closing && loop.timer_count() == 0,
    ...
    std::cout << "progress recv_bytes=" << partial.size() << " adopted_us=" << ticks(initial)
...
void write_progress(const hp::http::StaticFileService& service) {
    ...
    registry.add(Socket(pair.owned),
    ...
    require(::send(pair.peer.fd(), request.data(), request.size(), MSG_NOSIGNAL) ==
    ...
    require(Access::progress(connection) == progress && connection.pending_bytes() == queued,
    ...
    for (int cycle = 0; cycle < 12; ++cycle) {
        ...
        loop.poll_once(0);
        bool paced = false;
        loop.add_timer(Clock::now() + 30ms, [&] {
            paced = true;
        });
        while (!paced)
            loop.poll_once(-1);
    }
    require(Clock::now() - began > 150ms, "write progress survives original idle deadline");
    require(Access::progress(connection) > progress && connection.pending_bytes() < queued,
    ...
    while (!Access::entries(registry).empty())
        loop.poll_once(-1);
    require(ticks(Clock::now()) >= ticks(progress) + 150000, "stalled writer deadline");
    require(loop.timer_count() == 0, "stalled writer timer reclaimed");
    require(provider_calls == 1, "buffered suffix provider not called after timeout");
    std::string tail;
    for (;;) {
        auto n = ::recv(pair.peer.fd(), bytes, sizeof(bytes), 0);
        if (n <= 0)
    ...
    require(tail.find("408") == std::string::npos, "no synthetic timeout response");
    std::cout << "write received_bytes=" << received
              ...
              << " last_send_us=" << ticks(progress) << " closed_us=" << ticks(Clock::now())
              << " provider_calls=" << provider_calls << " timers=" << loop.timer_count() << '\n';
}
...
void paced_input(const hp::http::StaticFileService& service) {
    ...
    for (const std::string_view part :
        ...
        loop.poll_once(0);
        bool paced = false;
        loop.add_timer(Clock::now() + 50ms, [&] {
            paced = true;
        });
        while (!paced)
            loop.poll_once(-1);
        require(Access::entries(registry).size() == 1, "positive chunks keep connection alive");
    }
...
void healthy_during_timeout(const hp::http::StaticFileService& service) {
    ...
    const auto closed = ticks(Clock::now());
    require(after.timers == 0 && closed >= blocked.deadline, "production writer expiry reclaimed");
    harness.stop();
...
void rollback_and_reuse(const hp::http::StaticFileService& service) {
    ...
    for (int allocation = 0; allocation < 4; ++allocation) {
        ...
        timer_allocation_failure = -1;
        require(loop.timer_count() == 0 && Access::entries(registry).empty(), "adopt rollback");
        require(::fcntl(pair.owned, F_GETFD) == -1 && errno == EBADF, "adopt socket closed");
    }
    Pair original;
    registry.add(Socket(original.owned), hp::app::make_http_factory(service)());
    const auto old_identity = Access::entries(registry).at(original.owned)->identity();
    const auto old_timer = Access::timer(*Access::entries(registry).at(original.owned));
    ...
    registry.add(Socket(replacement.owned), hp::app::make_http_factory(service)());
    ...
    require(current.state() == TcpConnection::State::active && loop.timer_count() == 1,
            "old identity cannot close reused fd");
    bool callback_finished = false;
    loop.add_timer(Clock::now(), [&] {
        current.request_close();
        require(Access::entries(registry).size() == 1, "not destroyed inside callback");
        callback_finished = true;
    });
    loop.poll_once(0);
    require(callback_finished && Access::entries(registry).empty() && loop.timer_count() == 0,
    ...
    loop.reschedule_timer(Access::timer(*Access::entries(registry).at(eof.owned)), Clock::now());
    ::shutdown(eof.peer.fd(), SHUT_WR);
    loop.poll_once(0);
    require(Access::entries(registry).empty() && loop.timer_count() == 0, "EOF and expiry once");
    std::cout << "rollback post_registration_failures=" << failures_seen
              << " reused_fd=" << replacement.owned << " old_id=" << old_identity
              << " callback_finished=" << callback_finished << " remaining=" << loop.timer_count()
              << '\n';
}
...
void fatal_timer(const hp::http::StaticFileService& service) {
    ...
    require(TcpServerTestAccess::post(*harness.server, 1,
            ...
            "block other worker");
    require(entered.get_future().wait_for(3s) == std::future_status::ready, "other worker entered");
    ...
    require(TcpServerTestAccess::post(*harness.server, 0,
                                      [&](EventLoop& loop) {
                                          ...
                                          loop.add_timer(Clock::now() + 1h, [&, capture] {
                                              ++forbidden;
                                          });
                                      }),
            "install real worker timers");
    await([&] {
        return Access::stopping(*harness.server);
    });
    release.set_value();
    harness.failed("fatal timer original");
    require(callbacks == 1 && forbidden == 0 && captures == 1, "fatal timer terminal cleanup");
    first.eof();
    second.eof();
...
void active_lifecycle(const hp::http::StaticFileService& service) {
    ...
    for (int cycle = 0; cycle < 100; ++cycle) {
        ...
        for (std::size_t index = 0; index < 2; ++index) {
            ...
            require(state.connections == 1 && state.timers == 1, "active timer per worker");
            timers += state.timers;
        }
        harness.stop();
        first.eof();
    ...
    std::cout << "active_lifecycle cycles=100 observed_timers=" << timers << " fd=" << fd << '/'
...
void options_boundaries() {
    ...
    require(Access::config(component).idle == 0ms && Access::config(component).keep_alive == 0ms,
    ...
    for (const auto option : {"--idle-timeout-ms", "--keep-alive-timeout-ms"}) {
        for (const auto value : {"0", "1", "86400000"}) {
            auto config = parse({"server", "--port", "0", "--root", ".", option, value});
            require((std::string_view(option) == "--idle-timeout-ms" ? config.timeouts.idle
        ...
        for (const auto& values : {std::vector<std::string>{"server", option},
    ...
    std::cout << "options defaults_ms=" << defaults.timeouts.idle.count() << '/'
              << defaults.timeouts.keep_alive.count() << " cpp_ms=0/0 accepted=" << accepted
              << " rejected=" << rejected << '\n';
}
} // namespace
...
int main() {
    try {
        ...
        require(root_errors == 0 && live_sessions == 0, "service outlives timer callbacks");
        require(accepted_sockets == closed_sockets && invalid_closes == 0, "exact socket closes");
