...
void interleaved_and_eof() {
    ...
    auto provider = [](const http::HttpRequest& r,
...
void limits_and_500() {
    EventLoop loop;
    auto provider = [](const http::HttpRequest&,
                       http::ConnectionPolicy policy) {
    ...
    for (bool over : {false, true}) {
        ...
        expect(
            got == (over ? http::make_error_response(http::Status::bad_request)
                         : response("ok")),
...
void factory_message_lifetime() {
    ...
    int factory_errors{}, message_errors{}, callback_alive{}, recovered{};
    {
        ...
        if (fresh.owner.fd() != oldfd) {
            expect(::dup2(fresh.owner.fd(), oldfd) == oldfd,
                   "reuse numeric fd");
        ...
        S::drain(server);
        expect(S::size(server) == 1 &&
                   C::messages(S::connection(server, oldfd)) == 0,
               "old close/token no misdelete");
        fresh.send("ok");
        S::loop(server).poll_once(250);
        expect(
            collect(fresh.peer.fd()) ==
    ...
    std::cout << "failures: factory=" << factory_errors
              ...
              << " callback_alive=" << callback_alive
              << " recovery=" << recovered << " output_error=1 fd=" << before
              << "->" << fds() << '\n';
}
...
void reset_new_message() {
    ...
    expect(combined && result.socket_error == ECONNRESET &&
               result.bytes_read == 1053 && received == queued &&
               messages > 0 && closed == 1,
