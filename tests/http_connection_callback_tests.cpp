...
struct TcpConnectionTestAccess {
    ...
    static auto mask(TcpConnection& c) {
        return c.last_mask_;
    }

    static auto messages(TcpConnection& c) {
        return c.message_count_;
    }

    static auto buffered(TcpConnection& c) {
        return c.io_.input_view().size();
    }
    ...
    static auto token(TcpConnection& c) {
        return c.channel_.token();
    }
...
void interleaved_and_eof() {
    ...
    auto provider = [](const http::HttpRequest& r, http::ConnectionPolicy policy) {
        return http::ResponseResult{response(r.target), policy};
    };
    int closed{};
    TcpConnection ca(loop, std::move(a.owner), 1, app::make_http_callback(provider, &sa),
                     http::max_request_bytes, [&](int, auto) {
                         ++closed;
                     });
    TcpConnection cb(loop, std::move(b.owner), 2, app::make_http_callback(provider, &sb),
                     http::max_request_bytes, [&](int, auto) {
                         ++closed;
                     });
    ca.start();
    ...
    b.send("\r\n");
    loop.poll_once(250);
    expect(collect(a.peer.fd()) == response("/one") && collect(b.peer.fd()) == response("/two"),
           "interleaved exact distinct responses");
    expect(sa.responses == 1 && sb.responses == 1 && sa.need_more == 1 && sb.need_more == 1 &&
               closed == 2,
           "per-connection done state");
    Pair eof;
    app::HttpCallbackStats se;
    TcpConnection ce(loop, std::move(eof.owner), 3, app::make_http_callback(provider, &se),
                     http::max_request_bytes, [](int, auto) {
                     });
    ce.start();
    ...
    loop.poll_once(250);
    expect(collect(eof.peer.fd()) == http::make_error_response(http::Status::bad_request) &&
    ...
    std::cout << "isolation: callbacks=" << sa.callbacks + sb.callbacks
              << " need_more=" << sa.need_more + sb.need_more << " unique_responses=" << closed
              << " eof400=" << se.responses << " empty_eof=" << eof_empty << '\n';
}
...
void incremental_consumption() {
    ...
    TcpConnection ca(loop, std::move(a.owner), 90, app::make_http_callback(provider, &sa),
                     http::max_request_bytes, [](int, auto) {
                     });
    TcpConnection cb(loop, std::move(b.owner), 91, app::make_http_callback(provider, &sb),
                     http::max_request_bytes, [](int, auto) {
                     });
    ca.start();
    cb.start();
    ...
    for (std::size_t i = 0; i < pa.size(); ++i) {
        ...
        sent_a += pa[i].size();
        loop.poll_once(250);
        expect(sa.parses == i + 1 && sa.accepted_bytes == sent_a && sa.submitted_bytes == sent_a &&
                   sa.consumed_bytes == sent_a && C::buffered(ca) == 0,
               "each A segment fed once and released before next write");
        b.send(pb[i]);
        sent_b += pb[i].size();
        loop.poll_once(250);
        expect(sb.parses == i + 1 && sb.accepted_bytes == sent_b && sb.submitted_bytes == sent_b &&
                   sb.consumed_bytes == sent_b && C::buffered(cb) == 0,
    ...
    expect(collect(a.peer.fd()) == response("/a") && collect(b.peer.fd()) == response("/b") &&
               sa.need_more == 2 && sb.need_more == 2,
           "three fragments preserve independent request fields");
    Pair empty;
    app::HttpCallbackStats se;
    TcpConnection ce(loop, std::move(empty.owner), 92, app::make_http_callback(provider, &se),
                     http::max_request_bytes, [](int, auto) {
                     });
    ce.start();
    ::shutdown(empty.peer.fd(), SHUT_WR);
    loop.poll_once(250);
    expect(collect(empty.peer.fd()) == http::make_error_response(http::Status::bad_request) &&
               se.accepted_bytes == 0 && se.eof_notifications == 1,
           "empty HTTP EOF feed produces400 without consumed bytes");
    std::cout << "incremental: feeds=" << sa.parses + sb.parses
...
void limits_and_500() {
    EventLoop loop;
    auto provider = [](const http::HttpRequest&, http::ConnectionPolicy policy) {
    ...
    for (bool over : {false, true}) {
        ...
        TcpConnection c(loop, std::move(p.owner), 5, app::make_http_callback(provider, &st),
                        http::max_request_bytes, [](int, auto) {
                        });
        c.start();
        std::string input = over ? std::string(http::max_request_bytes + 1, 'a') : exact;
        p.send(input);
        loop.poll_once(250);
        auto got = collect(p.peer.fd());
        expect(got ==
                   (over ? http::make_error_response(http::Status::bad_request) : response("ok")),
    ...
    TcpConnection c(
        ...
        app::make_http_callback(
            [](const http::HttpRequest&, http::ConnectionPolicy) -> http::ResponseResult {
    ...
    p.send(request);
    loop.poll_once(250);
    expect(collect(p.peer.fd()) == http::make_error_response(http::Status::internal_server_error) &&
               st.responses == 1,
    ...
    bounded.send("12345");
    loop.poll_once(250);
    expect(exact_seen == 1 && limit_closed == 1 && C::result(cap).read_error == EMSGSIZE,
           "notify exact cap before controlled overflow close");
    std::cout << "limits: exact_http=1 over_http=1 exact_cap_notification=" << exact_seen
              << " cap_close=" << limit_closed << " adapter500=" << st.responses << '\n';
}
...
void drain_pipeline_and_borrow() {
    ...
    int closes{};
    TcpConnection c(loop, std::move(p.owner), 8,
                    app::make_http_callback(
    ...
    expect(pending > 0 && C::result(c).write_would_block && closes == 0,
           "close_after_flush retains temporary response tail");
    ...
    for (int i = 0; i < 10; ++i)
        loop.poll_once(0);
    expect(stats.parses == parses && stats.responses == sends && stats.callbacks == callbacks &&
               C::events(c) == events,
    ...
    std::cout << "drain: pending=" << pending << " eagain=1 full_bytes=" << received.size()
...
void factory_message_lifetime() {
    ...
    {
        int factories{};
        TcpServer server(0, [&]() -> TcpConnection::MessageCallback {
            if (++factories == 1) {
                ++factory_errors;
        ...
        S::drain(server);
        expect(S::size(server) == 1 && C::messages(S::connection(server, oldfd)) == 0,
        ...
        Pair output;
        TcpConnection c(S::loop(server), std::move(output.owner), 77, {}, 0, [](int, auto) {
        });
        c.start();
        output.peer.reset();
        c.send(bytes("out"));
        expect(c.state() == TcpConnection::State::closing, "send output error closes");
        Pair active;
    ...
    expect(fds() == before && factory_errors == 1 && message_errors == 1 && callback_alive == 1 &&
               recovered == 1,
           "failure lifecycle and fd restoration");
    std::cout << "failures: factory=" << factory_errors << " message=" << message_errors
...
void reset_new_message() {
    EventLoop loop;
    Socket accepted;
    Acceptor acceptor(loop, 0, [&](Socket s) {
        accepted = std::move(s);
    });
    acceptor.start();
    ...
    expect(::connect(client.fd(), reinterpret_cast<sockaddr*>(&address), sizeof(address)) == 0,
           "reset TCP connect");
    loop.poll_once(250);
    acceptor.stop();
    std::vector<std::byte> received;
    ...
    expect(::send(client.fd(), queued.data(), queued.size(), MSG_NOSIGNAL) == 1053,
           "reset queue bytes");
    ...
    expect(combined && result.socket_error == ECONNRESET && result.bytes_read == 1053 &&
               received == queued && messages > 0 && closed == 1,
           "new callback receives all diagnosed reset bytes");
    std::cout << "reset: combined=" << combined << " so_error=" << result.socket_error
