...
void reuse(std::uint16_t port) {
  ...
  }
  {
    ...
    for (std::size_t i = 0; i < 3; ++i) check(c.next(), 200, bodies[i], i == 2);
    c.eof();
  ...
  }
  {
    ...
    for (std::size_t i = 0; i < 3; ++i) check(c.next(), 200, bodies[i], false);
    c.eof();
  }
  {
    Client c(port);
  ...
  for (bool prefix : {false, true}) {
    ...
    send_all(c.fd, (prefix ? get("/") : "") + get("/bad%20target") +
                       get("/note.txt"));
    if (prefix) check(c.next(), 200, bodies[0], false);
    check(c.next(), 400, "400 Bad Request\n", true);
    c.eof();
  }
...
void rejection_positions(ServerProcess& server) {
  for (const auto& sample : protocol_cases::rejections())
    for (bool prefix : {false, true}) {
      ...
      if (prefix) check(c.next(), 200, "hello from S3\n", false);
...
int main(int argc, char** argv) {
  ...
  try {
    ...
    // usable.
    {
      ...
      linger reset{1, 0};
      expect(
          ::setsockopt(c.fd, SOL_SOCKET, SO_LINGER, &reset, sizeof(reset)) == 0,
