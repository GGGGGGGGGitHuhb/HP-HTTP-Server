...
bool limit_termination_ok(std::string_view pending, ssize_t received, int error) {
    return pending.empty() && (received == 0 || (received == -1 && error == ECONNRESET));
}
...
public:
    ...
    Message next() {
        ...
        expect(pos != std::string::npos && header.find(key, pos + 1) == std::string::npos,
    ...
    void eof() {
        ...
        char b;
        expect(::recv(fd, &b, 1, 0) == 0, "terminal response followed by real EOF");
    }
};
...
void check(const Client::Message& m, int status, std::string_view body, bool close) {
...
void reuse(std::uint16_t port) {
                                             ...
                                             "unknown mime\n"};
    {
        ...
        for (std::size_t i = 0; i < 3; ++i) {
            send_all(c.fd, get(targets[i], i == 2 ? "Connection: close\r\n" : ""));
            check(c.next(), 200, bodies[i], i == 2);
        }
        c.eof();
        std::cout << "sequential: client_fd=" << c.fd << " connection=1 responses=3 reconnect=0\n";
    }
    {
        Client c(port);
        send_all(c.fd,
                 get(targets[0]) + get(targets[1]) + get(targets[2], "Connection: close\r\n"));
        for (std::size_t i = 0; i < 3; ++i)
            check(c.next(), 200, bodies[i], i == 2);
        c.eof();
        std::cout << "pipeline: connection=1 single_write=1 responses=3 order=index,note,unknown\n";
    }
    {
        Client c(port);
        send_all(c.fd,
                 get(targets[0]) + get(targets[1]) + "GET /assets/unknown.blob HTTP/1.1\r\nHo");
    ...
    {
        Client c(port);
        send_all(c.fd, get("/") + get("/note.txt") + get("/assets/unknown.blob"));
    ...
    for (bool prefix : {false, true}) {
        Client c(port);
        send_all(c.fd, (prefix ? get("/") : "") + get("/bad%20target") + get("/note.txt"));
        if (prefix)
            check(c.next(), 200, bodies[0], false);
        check(c.next(), 400, "400 Bad Request\n", true);
        c.eof();
    ...
    for (const auto& header :
        ...
        check(c.next(), normal ? 200 : 400, normal ? bodies[1] : "400 Bad Request\n", true);
        c.eof();
    }
    std::cout
...
void termination_guard_tests() {
    expect(limit_termination_ok("", 0, 0) && limit_termination_ok("", -1, ECONNRESET),
    ...
    for (int error : {EAGAIN, EWOULDBLOCK, EIO, ETIMEDOUT, EINTR})
        expect(!limit_termination_ok("", -1, error), "other errors never accepted as reset");
    for (bool short_body : {true, false}) {
        ...
        expect(::shutdown(writer.fd, SHUT_WR) == 0, "termination fixture shutdown");
        bool rejected = false;
        try {
            (void)client.next();
            if (short_body)
        ...
        expect(rejected, short_body ? "short response fails before termination rule"
...
void rejection_positions(ServerProcess& server) {
    for (const auto& sample : protocol_cases::rejections())
        for (bool prefix : {false, true}) {
            ...
            send_all(c.fd,
                     (prefix ? get("/note.txt") : "") + sample.wire + get("/assets/unknown.blob"));
            if (prefix)
...
void fin_prefixes(ServerProcess& server) {
    ...
    for (bool reused : {false, true})
        for (std::size_t cut = 1; cut < protocol_cases::short_request.size(); ++cut) {
            ...
            send_all(c.fd, std::string_view(protocol_cases::short_request).substr(0, cut));
            expect(::shutdown(c.fd, SHUT_WR) == 0, "prefix FIN sent");
            check(c.next(), 400, "400 Bad Request\n", true);
            c.eof();
            ++cases;
            std::cout << "S3 FIN cut=" << cut << " state=" << protocol_cases::prefix_state(cut)
...
void reset_incomplete(ServerProcess& server, std::size_t baseline) {
    {
        ...
        expect(::poll(&descriptor, 1, 0) == 0, "incomplete request has no premature response");
        linger reset{1, 0};
        expect(::setsockopt(incomplete.fd, SOL_SOCKET, SO_LINGER, &reset, sizeof(reset)) == 0,
    ...
    expect(wait_fd_baseline(server, baseline), "incomplete RST fd recovery by deadline");
...
int main(int argc, char** argv) {
    ...
    try {
        ...
        // Reset a paused large response, then prove other connections remain usable.
        {
            ...
            const auto until = std::chrono::steady_clock::now() + std::chrono::seconds(5);
            while (server.output().find("write reached EAGAIN", start) == std::string::npos &&
                   std::chrono::steady_clock::now() < until)
                server.drain_output(std::chrono::milliseconds(20));
            expect(server.output().find("write reached EAGAIN", start) != std::string::npos,
                   "fresh EAGAIN before large-response RST");
            linger reset{1, 0};
            expect(::setsockopt(c.fd, SOL_SOCKET, SO_LINGER, &reset, sizeof(reset)) == 0,
        ...
        expect(wait_fd_baseline(server, before), "large RST deadline fd recovery");
        const auto after = server.fd_count();
        expect(before == after, "production fd baseline restored after reset and 20 survivors");
