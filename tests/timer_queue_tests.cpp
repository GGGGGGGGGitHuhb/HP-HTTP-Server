...
void queue_boundaries() {
    ...
    const auto first = queue.add(zero + 10ms, [&] {
        fired.push_back(1);
    });
    const auto second = queue.add(zero + 10ms, [&] {
        fired.push_back(2);
    });
    ...
    for (int i = 0; i < 100000; ++i)
        check(queue.reschedule(id, zero + std::chrono::milliseconds(i % 500 + 1)),
              "reschedule active");
    check(queue.size() == 1 && hp::timer::TimerQueueTestAccess::ordered(queue) == 1,
...
void allocation_safety() {
    ...
    for (int allocation : {0, 1}) {
        ...
        allocation_countdown = -1;
        check(queue.size() == 0 && hp::timer::TimerQueueTestAccess::ordered(queue) == 0,
    ...
    allocation_countdown = -1;
    check(queue.size() == 1 && queue.next_deadline() == zero + 10ms, "renewal strong guarantee");
    queue.clear();
    std::cout << "allocation: add_indices=2 reschedule=1 strong_guarantee=verified\n";
}
...
void event_loop() {
    ...
    auto old = loop.add_timer(TimerQueue::Clock::now(), [&] {
        fired += 100;
    });
    ...
    std::cout << "event_loop: fired=" << fired << " pure_timer_cleanup=1 IO_first=verified\n";
}
