...
void chunk_variants(const std::string& input, ParseStatus expected,
    ...
    for (auto split : points) {
        ...
        if (first.status == ParseStatus::need_more)
            expect(first.accepted_bytes == split,
                   "NeedMore accepts full block");
    }
    RequestParser p;
    FeedResult last;
    std::size_t total{};
...
void support_and_splits() {
    ...
    std::cout << "matrix: samples=" << samples.size()
              << " split_cases=" << splits << " byte_feeds=" << byte_feeds
              << " cr_splits=" << cr_splits << '\n';
}
...
void boundaries() {
    for (std::size_t length : {4095U, 4096U, 4097U}) {
        std::string line =
            "GET /" + std::string(length - 14, 'a') + " HTTP/1.1";
    ...
    auto limit = p.feed("b");
    expect(limit.status == ParseStatus::bad_request &&
               limit.request_bytes == 16384,
           "unterminated cap rejects exactly");
    std::cout
        << "bounds: line4095/4096/4097=pass total16383/16384/16385=pass max_scan_visits="
...
void sticky_reset_and_ownership() {
    ...
    auto error = p.feed("X");
    expect(
        error.status == ParseStatus::bad_request && error.accepted_bytes == 1,
    ...
    expect(recovered.status == ParseStatus::complete &&
               recovered.accepted_bytes == one.size() &&
...
void named_schedule(const NamedCase& sample,
    ...
    for (auto count : chunks) {
        result =
            parser.feed(std::string_view(sample.wire).substr(offset, count));
        expect(result.accepted_bytes <= count,
               "per-feed accepted <= submitted");
        accepted += result.accepted_bytes;
        offset += count;
        observe(parser, result);
        expect(result.request_bytes == accepted,
               "per-feed cumulative consumption");
    ...
    for (const auto& [id, header] : rejected)
        for (bool nonget : {false, true}) {
            const std::string prefix = std::string(nonget ? "POST" : "GET") +
                                       " /case HTTP/1.1\r\n" + host +
                                       "Connection: close\r\n";
            bad("G03-04-" + id + (nonget ? "-405-priority" : "-after-close"),
                prefix + header,
                id == "te-cl" ? "Content-Length: 0\r\n\r\n" : "\r\n");
        }
    for (const auto& sample : cases)
        named_variants(sample);
    std::cout << "S3 named_matrix=" << cases.size()
              << " schedules=" << named_runs << " seeded=" << seeded_runs
              << " seeds=0x13579,0x24680\n";
}
...
void named_boundaries_and_reset() {
    ...
    std::cout
        << "S3 limits: many_headers=3 max_requests_total=32768 reset_pending_CR=1 named_runs="
