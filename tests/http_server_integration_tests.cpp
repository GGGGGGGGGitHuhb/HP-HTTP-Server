  ...
  public:
    Fixture() {
        ...
        const std::filesystem::path base =
            configured == nullptr ? std::filesystem::path(".cache/olympus-v0.1-s3/tests")
    ...
    static void write_text(const std::filesystem::path& path, std::string_view data) {
    ...
    static void write_bytes(const std::filesystem::path& path, const std::vector<std::byte>& data) {
        std::ofstream output(path, std::ios::binary);
        output.write(reinterpret_cast<const char*>(data.data()),
                     static_cast<std::streamsize>(data.size()));
        if (!output)
            throw std::runtime_error("fixture binary write failed");
    }
...
class ServerProcess {
  public:
    ServerProcess(pid_t pid, int output_fd, std::uint16_t port, std::string output)
        : pid_(pid), output_fd_(output_fd), port_(port), output_(std::move(output)) {}
    ...
    ServerProcess(ServerProcess&& other) noexcept
        : pid_(std::exchange(other.pid_, -1)), output_fd_(std::exchange(other.output_fd_, -1)),
          port_(other.port_), output_(std::move(other.output_)) {}
    ...
    [[nodiscard]] std::size_t fd_count() const {
        std::size_t count = 0;
        const std::filesystem::path fd_root = "/proc/" + std::to_string(pid_) + "/fd";
    ...
    [[nodiscard]] std::size_t thread_count() const {
        return static_cast<std::size_t>(std::distance(
            std::filesystem::directory_iterator("/proc/" + std::to_string(pid_) + "/task"), {}));
    }
...
ServerProcess start_server(const char* executable, const std::filesystem::path& root,
    ...
    if (flags == -1 || ::fcntl(output_pipe[0], F_SETFL, flags | O_NONBLOCK) == -1) {
        const int error_number = errno;
        ::kill(child, SIGKILL);
        (void)::waitpid(child, nullptr, 0);
        ::close(output_pipe[0]);
        throw std::runtime_error(std::string("fcntl: ") + std::strerror(error_number));
    }
    ...
    while (std::chrono::steady_clock::now() < deadline) {
        ...
        if (marker_position != std::string::npos) {
            ...
            const unsigned long parsed = std::stoul(output.substr(start, end - start));
            if (parsed > 0 && parsed <= 65535) {
                return ServerProcess(child, output_pipe[0], static_cast<std::uint16_t>(parsed),
                                     output);
            }
        }
        int status = 0;
...
int connect_client(std::uint16_t port, int receive_buffer = 0) {
    ...
    if (::connect(fd, reinterpret_cast<const sockaddr*>(&address), sizeof(address)) == -1) {
        const int error_number = errno;
        ::close(fd);
        throw std::runtime_error(std::string("connect: ") + std::strerror(error_number));
    }
    return fd;
}
...
std::vector<std::byte>
transact(std::uint16_t port, std::string_view request, int receive_buffer = 0,
         std::chrono::milliseconds pause_before_read = std::chrono::milliseconds(0)) {
...
Response parse_response(const std::vector<std::byte>& bytes) {
    ...
    expect(length_start != std::string::npos, "every response must include Content-Length");
    if (length_start != std::string::npos) {
        ...
        expect(declared == bytes.size() - boundary - 4, "Content-Length must equal body bytes");
    }
...
Response request(std::uint16_t port, std::string_view target, std::string_view method = "GET",
...
void test_success(std::uint16_t port, const Fixture& fixture) {
    ...
    expect(index.status == 200 && body_text(index) == "<h1>integration index</h1>\n",
...
void test_errors_and_paths(std::uint16_t port) {
    expect(request(port, "/missing.txt").status == 404, "missing file must return 404");
    const Response post = request(port, "/", "POST");
    expect(post.status == 405 && post.header.find("Allow: GET\r\n") != std::string::npos,
    ...
    for (const auto& raw : bad_requests) {
        expect(parse_response(transact(port, raw)).status == 400, "bad request must return 400");
    }
    ...
    for (const auto& [target, status] : paths) {
        ...
        expect(response.status == status, "path rejection must use its designed status");
...
void test_segmented_limits_and_half_close(std::uint16_t port) {
    ...
    expect(request_at_limit.size() == request_limit, "limit fixture must be exactly 16 KiB");
    expect(parse_response(transact(port, request_at_limit)).status == 400,
           "16 KiB incomplete headers must return 400");

    const Response half_closed = parse_response(
        transact(port, "GET / HTTP/1.1\r\nHost: localhost\r\nConnection: close\r\n"));
    ...
    expect(pipelined.status == 200 &&
               raw.find("HTTP/1.1", raw.find("HTTP/1.1") + 1) == std::string::npos,
           "pipelined bytes must produce exactly one response");
}

void test_short_write_and_isolation(ServerProcess& server, const Fixture& fixture) {
    const std::string raw =
        "GET /large.bin HTTP/1.1\r\nHost: localhost\r\nConnection: close\r\n\r\n";
    const Response large =
        parse_response(transact(server.port(), raw, 4096, std::chrono::milliseconds(300)));
    ...
    if (server.output().find("S3 evidence: connection write reached EAGAIN") != std::string::npos) {
        ++write_eagain_hits;
    }
    expect(write_eagain_hits > 0, "paused client must dynamically reach production write EAGAIN");

    const int reset_client = connect_client(server.port());
    linger reset_linger{1, 0};
    (void)::setsockopt(reset_client, SOL_SOCKET, SO_LINGER, &reset_linger, sizeof(reset_linger));
...
int main(int argc, char* argv[]) {
    ...
    std::cout << "HTTP integration tests passed; status_hits={200:" << status_200_hits
              << ",400:" << status_400_hits << ",403:" << status_403_hits
              << ",404:" << status_404_hits << ",405:" << status_405_hits << "}"
              << " segmented_need_more_hits=" << segmented_need_more_hits
              << " write_eagain_hits=" << write_eagain_hits
              << " path_secret_leaks=0 accept_drain_successes=" << accept_drain_successes
