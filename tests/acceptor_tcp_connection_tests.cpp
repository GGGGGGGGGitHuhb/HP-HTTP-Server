...
struct TcpConnectionTestAccess {
    ...
    static auto pending(TcpConnection& c) {
        return c.io_.pending_bytes();
    }
...
struct AcceptorTestAccess {
    static int fd(Acceptor& a) {
        return a.listener_.fd();
    }

    static void invalidate_listener(Acceptor& a) {
        a.listener_.reset();
    }
};

struct TcpServerTestAccess {
    static EventLoop& loop(TcpServer& s) {
        return s.loop_;
    }
...
Socket connect_to(std::uint16_t port) {
    ...
    if (!client.valid() ||
        ::connect(client.fd(), reinterpret_cast<sockaddr*>(&address), sizeof(address)))
...
void acceptor_delivery() {
    ...
    int accepted{}, transferred{}, failures_seen{}, callbacks_after_stop{}, bind_failure{},
        initial_drain{};
    {
        EventLoop loop;
    ...
    std::cout << "acceptor: initial_drain=" << initial_drain << " accepted=" << accepted
              << " transferred=" << transferred << " delivery_failure=" << failures_seen
              << " bind_failure=" << bind_failure << " post_stop=" << callbacks_after_stop
              << " duplicate=0 fd=" << before << "->" << fd_count() << '\n';
}
...
void buffers_and_echo() {
    ...
    Pair pair;
    int size = 4096;
    expect(::setsockopt(pair.observed.fd(), SOL_SOCKET, SO_SNDBUF, &size, sizeof(size)) == 0,
           "small send buffer");
    int closes{};
    ...
    expect(C::result(c).write_would_block && pending > 0 && pending < payload.size() &&
    ...
    expect(received == payload && C::pending(c) == 0 && !(C::interest(c) & EPOLLOUT),
...
void application_boundaries() {
    ...
    expect(n == 2 && bytes[0] == std::byte{0x78} && bytes[1] == std::byte{0x79} && calls == 2 &&
               notices == 1,
    ...
    std::cout << "application: segmented_calls=" << calls
              << " one_response=1 limit_calls=" << limit_calls << " limit_close=" << limit_closed
              << '\n';
}
...
void close_batch_and_reuse() {
    ...
    std::size_t stale{};
    {
        ...
        expect(self_calls == 1 && survivor_calls == 1 && stale == 1 && S::size(server) == 1,
    ...
    expect(fd_count() == before, "server destructor restores active connection resources");
    {
        ...
        S::close_notice(server, fd, old_id);
        S::drain(server);
        expect(S::size(server) == 1 && ::fcntl(fd, F_GETFD) >= 0 && C::events(current) == 0,
    ...
    std::cout << "lifecycle: callback_alive=" << in_callback_fd_alive << " self=" << self_calls
              << " survivor=" << survivor_calls << " same_batch_stale=" << stale << " fd=" << before
              << "->" << fd_count() << '\n';
}
...
void failures_and_recovery() {
    ...
    int constructor_fail{}, listener_add_fail{}, add_fail{}, mod_fail{}, recovery{};
    {
        ...
        try {
            TcpConnection bad(loop, std::move(pair.observed), 0, {}, 0, [](int, auto) {
            });
    ...
    }
    {
        ...
        if (S::size(server) == 0 && ::fcntl(fd, F_GETFD) == -1)
            ++mod_fail;
    ...
    std::cout << "failures: constructors=" << constructor_fail
              << " listener_add=" << listener_add_fail << " add=" << add_fail << " mod=" << mod_fail
              << " recovery=" << recovery << " fd=" << before << "->" << fd_count() << '\n';
}
...
void real_reset() {
    ...
    expect(combined && result.socket_error_observed && result.socket_error == ECONNRESET &&
               result.bytes_read == bytes.size() && notices == 1,
           "TcpConnection full ERR/IN -> SO_ERROR -> exact recv -> deferred close");
    std::cout << "reset: tcp_connection_callbacks=" << C::events(c) << " combined=" << combined
