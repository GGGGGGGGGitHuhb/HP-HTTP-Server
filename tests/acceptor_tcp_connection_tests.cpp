...
void await_accept_queue(Acceptor& acceptor, unsigned expected) {
    ...
    for (int attempt = 0; attempt < 1000; ++attempt) {
        ...
        if (::getsockopt(AcceptorTestAccess::fd(acceptor), IPPROTO_TCP,
                         TCP_INFO, &info, &length))
...
void buffers_and_echo() {
    ...
    expect(received == payload && C::pending(c) == 0 &&
...
void application_boundaries() {
    ...
    expect(n == 2 && bytes[0] == std::byte{0x78} &&
               bytes[1] == std::byte{0x79} && calls == 2 && notices == 1,
...
void close_batch_and_reuse() {
    ...
    std::size_t stale{};
    {
        ...
        TcpServer server(0, [&] {
            return [&](TcpConnection&, std::span<const std::byte> bytes, bool) {
                ...
                a->request_close();
                b->request_close();
                if (::fcntl(a->fd(), F_GETFD) >= 0 &&
                    ::fcntl(b->fd(), F_GETFD) >= 0)
           ...
           "server destructor restores active connection resources");
    {
        ...
        if (fresh.observed.fd() != fd) {
            expect(::dup2(fresh.observed.fd(), fd) == fd,
                   "force same numeric fd");
        ...
        fresh.ready();
        S::loop(server).poll_once(250);
        expect(C::events(current) == 1,
               "new real readiness reaches TcpConnection");
        std::cout
            << "reuse: fd=" << fd << " old_id=" << old_id
...
void failures_and_recovery() {
        ...
        recovery{};
    {
        ...
        expect(::fcntl(fd, F_GETFD) == -1,
               "failed constructor reclaims socket");
        try {
        ...
        expect(
            unsupported.state() == TcpConnection::State::unregistered &&
    ...
    }
    {
        ...
        expect(::poll(&readable, 1, 1000) == 1,
               "await actual TCP echo delivery");
        if (::recv(client.fd(), output, 2, MSG_DONTWAIT) == 2 &&
...
void real_reset() {
    ...
    expect(
        ::send(client.fd(), bytes.data(), bytes.size(), MSG_NOSIGNAL) == 1053,
