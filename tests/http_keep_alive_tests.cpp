...
struct TcpConnectionTestAccess {
    ...
    static auto result(TcpConnection& c) {
        return c.last_result_;
    }

    static auto events(TcpConnection& c) {
        return c.event_count_;
    }

    static auto interest(TcpConnection& c) {
        return c.channel_.interest();
    }
};
}
...
std::string request(std::string_view target = "/a", std::string_view header = "") {
...
std::string response(std::string_view body,
    ...
    return text(http::make_response(http::Status::ok, bytes(body), "text/plain", false, policy));
}
...
void pipeline_slow_and_repeated() {
    ...
    TcpConnection c(
        ...
        app::make_http_callback([&](const http::HttpRequest& r, http::ConnectionPolicy policy) {
            ++calls;
            return http::ResponseResult{
                http::make_response(http::Status::ok, bytes(r.target == "/big" ? big : r.target),
    ...
    const auto pending = c.pending_bytes(), buffered = c.input_view().size(), events = C::events(c);
    ...
    for (int i = 0; i < 10; ++i)
        loop.poll_once(0);
    expect(calls == 1 && c.input_view().size() == buffered && C::events(c) == events,
    ...
    p.send(many);
    expect(pump(loop, p, c, expected_many.size()) == expected_many && calls == 628,
    ...
    loop.poll_once(100);
    expect(closes == 1 && p.collect().empty(), "idle EOF after requests closes silently once");
    std::cout << "slow: EAGAIN=" << eagain << " blocked_provider=1 suffix=" << buffered
              << " initial_pending=" << pending << " max_pending=" << max_pending
              << " cumulative_short_bytes=" << many.size() << " output_capacity=" << capacity
              << " requests=" << calls << " idle_polls=20 close=" << closes << '\n';
}
...
void terminal_matrix_and_eof() {
    ...
    std::size_t cases{};
    auto run = [&](const std::string& input, const std::string& wanted, std::size_t expected_calls,
                   bool eof) {
        EventLoop loop;
        Pair p;
        ...
        TcpConnection c(
            ...
            app::make_http_callback([&](const http::HttpRequest& r, http::ConnectionPolicy policy) {
                ...
                if (r.target == "/403" || r.target == "/404" || r.target == "/500")
                    return http::ResponseResult{
                        http::make_error_response(r.target == "/403" ? http::Status::forbidden
                                                  : r.target == "/404"
                                                      ? http::Status::not_found
                                                      : http::Status::internal_server_error,
                                                  policy),
                        policy};
                return http::ResponseResult{http::make_response(http::Status::ok, bytes(r.target),
                                                                "text/plain", false, policy),
                                            policy};
            }),
            http::max_request_bytes, [&](int, auto) {
        ...
        for (int i = 0; i < 5; ++i)
            loop.poll_once(0);
        expect(got == wanted && p.collect().empty() && calls == expected_calls && closes == 1,
    ...
    for (const auto& h : invalid)
        run(request("/a") + request("/bad", h) + request("/suffix"), response("/a") + bad, 1,
            false);
    for (const auto& h : {"Connection: ClOsE,keep-alive\r\nConnection: keep-alive\r\n",
                          "Connection: keep-alive\r\nConnection: x, close\r\n"}) {
        run(request("/a", h) + request("/suffix"), response("/a", http::ConnectionPolicy::close), 1,
            false);
        run(request("/a") + request("/b", h) + request("/suffix"),
            response("/a") + response("/b", http::ConnectionPolicy::close), 2, false);
    }
    run(request("/a") + "POST / HTTP/1.1\r\nHost: a\r\n\r\n" + request("/suffix"),
        response("/a") + text(http::make_error_response(http::Status::method_not_allowed)), 1,
        false);
    run(request("/a") + request("/throw") + request("/suffix"),
        response("/a") + text(http::make_error_response(http::Status::internal_server_error)), 2,
        false);
    run("", bad, 0, true);
    ...
    for (const auto& target : {"/403", "/404", "/500"}) {
        auto status = std::string_view(target) == "/403"   ? http::Status::forbidden
                      : std::string_view(target) == "/404" ? http::Status::not_found
                                                           : http::Status::internal_server_error;
        run(request(target) + request("/b", "Connection: close\r\n"),
            text(http::make_error_response(status, http::ConnectionPolicy::keep_alive)) +
                response("/b", http::ConnectionPolicy::close),
            2, false);
    }
    std::cout << "terminal/eof: cases=" << cases << " rejected_framing=" << invalid.size()
              << " suffix_calls=0 duplicate_responses=0\n";
}

void service_policy_and_fin() {
    const auto base = std::filesystem::path(std::getenv("HP_S3_TEST_TMP_ROOT")
    ...
    for (bool prefix : {false, true}) {
        ...
        auto wanted =
            (prefix ? text(service.handle({"GET", "/"}, http::ConnectionPolicy::keep_alive)) : "") +
            text(http::make_error_response(http::Status::bad_request));
        expect(pump(loop, p, c, wanted.size()) == wanted && calls == (prefix ? 2U : 1U) &&
                   closed == 1,
    ...
    }
    {
        ...
        TcpConnection c(
            ...
            app::make_http_callback([&](const http::HttpRequest&, http::ConnectionPolicy) {
                ++calls;
                return http::ResponseResult{http::make_response(http::Status::ok, bytes("INVALID"),
                                                                "text/plain", false,
        ...
        expect(pump(loop, p, c, wanted.size()) == wanted && calls == 1 && closed == 1,
            ...
            app::make_http_callback([&](const http::HttpRequest& r, http::ConnectionPolicy policy) {
                ...
                return http::ResponseResult{
                    http::make_response(http::Status::ok,
                                        bytes(r.target == "/big" ? big : r.target), "text/plain",
                                        false, policy),
                    policy};
            }),
            http::max_request_bytes, [&](int, auto) {
...
void generic_drains() {
    for (int mode = 0; mode < 3; ++mode) {
        ...
        c.send(bytes("one"));
        expect(notifications == 0, "event-external send never synchronously notifies");
        auto got = pump(loop, p, c, 6);
        for (int i = 0; i < 10; ++i)
            loop.poll_once(0);
        expect(got == "onetwo" && notifications == 2 && max_depth == 1 && closed == 1,
    ...
    }
    {
        ...
        std::cout << "drain destruction: callback_alive=" << alive << " returned=" << returned
                  << " destroyed=" << destroyed << "\n";
    }
}
...
void boundary_rejection_and_fin() {
    ...
    for (const auto& sample : protocol_cases::rejections())
        for (bool prefix : {false, true}) {
            ...
            TcpConnection c(loop, std::move(pair.owner), 81,
            ...
            c.start();
            pair.send((prefix ? request("/prefix") : "") + sample.wire + request("/suffix"));
            const auto expected = (prefix ? response("/prefix") : "") +
            ...
            for (int i = 0; i < 5; ++i)
                loop.poll_once(0);
            expect(calls == static_cast<std::size_t>(prefix) + sample.reaches_service &&
                       suffix_calls == 0 && closes == 1 && pair.collect().empty(),
                   "S3 rejection invokes no suffix and closes once");
            std::cout << "S3 component reject=" << sample.id << " position=" << (prefix ? 2 : 1)
                      << " provider=" << calls << " suffix=" << suffix_calls << " closes=" << closes
                      << '\n';
            ++rejected;
        }
    for (bool reused : {false, true})
        for (std::size_t cut = 1; cut < protocol_cases::short_request.size(); ++cut) {
            EventLoop loop;
            Pair pair;
            std::size_t calls{}, closes{};
            TcpConnection c(loop, std::move(pair.owner), 82,
                            app::make_http_callback(
                                [&](const http::HttpRequest& r, http::ConnectionPolicy policy) {
                                    ++calls;
                                    return http::ResponseResult{
            ...
            if (reused) {
                ...
                expect(pump(loop, pair, c, response("/prefix").size()) == response("/prefix"),
            ...
            const auto expected = text(http::make_error_response(http::Status::bad_request));
            expect(pump(loop, pair, c, expected.size()) == expected,
    ...
    std::cout << "S3 component rejection_cases=" << rejected << " FIN_prefix_cases=" << truncated
