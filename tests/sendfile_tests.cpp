// Reuse the accepted owner/TID, process, HTTP and fault fixtures without copying transport logic.
...
struct FileEvidence {
    ...
    dev_t device;
    ino_t inode;
    std::size_t closes{}, reads{}, preads{}, stats{}, calls{}, positive{}, short_writes{}, eagain{};
...
int __wrap_pthread_sigmask(int how, const sigset_t *set, sigset_t *old) {
    if (mask_injection && how == SIG_BLOCK && set && ::sigismember(set, SIGPIPE) == 1) {
...
ssize_t __wrap_sendfile(int output, int input, off_t *offset, std::size_t count) {
    ...
    const int saved = errno;
    {
        ...
        if (auto item = active_files.find(input); item != active_files.end()) {
            ...
            if (injected) {
                ...
                record.eagain += result < 0 && (saved == EAGAIN || saved == EWOULDBLOCK);
            }
            record.offset_error |= !offset || *offset != before + (result > 0 ? result : 0);
        }
    }
    errno = saved;
    return result;
...
void raw_observation(const Fixture &fixture, int disabled_error) {
    int pair[2];
    require(::socketpair(AF_UNIX, SOCK_STREAM | SOCK_NONBLOCK, 0, pair) == 0, "observer pair");
    Socket output(pair[0]), input(pair[1]);
    auto file = open_fixture(fixture);
    const auto id = latest_file();
    off_t offset = 0;
    require(::sendfile(output.fd(), file.fd(), &offset, 1024) == 1024, "real sendfile observer");
    std::array<char, 1024> bytes;
    require(::recv(input.fd(), bytes.data(), bytes.size(), 0) == 1024, "raw observer receive");
    require(std::memcmp(bytes.data(), fixture.large.data(), bytes.size()) == 0, "raw bytes exact");
    for (int error : {EINTR, EIO, ENOSYS}) {
        ...
        require(result == -1 && actual == error && sendfile_injection_consumed == before + 1,
                "sendfile injection must be consumed");
    }
    const auto record = evidence(id);
    require(record.positive == 1 && record.bytes == 1024 && !record.offset_error &&
                record.reads == 0 && record.preads == 0 && record.injected == 3,
            "observer distinguishes real syscall from injection");
    std::cout << "M0 observer real_calls=" << record.calls << " bytes=" << record.bytes
              << " injected=" << record.injected << " read/pread=0/0 offset=" << offset << '\n';
}
...
struct ConnectionIoTestAccess {
    static std::size_t capacity(const ConnectionIo &io) { return io.output_.capacity(); }
};
} // namespace hp::net
...
static_assert(!std::is_copy_constructible_v<FileRegion> && !std::is_copy_assignable_v<FileRegion>);
static_assert(std::is_nothrow_move_constructible_v<FileRegion> &&
              std::is_nothrow_move_assignable_v<FileRegion>);

FileRegion region(const Fixture &fixture, std::size_t length, off_t offset = 0) {
    auto file = open_fixture(fixture);
...
void collect(int fd, std::string &target) {
    ...
    for (;;) {
        ...
        require(count == 0 || errno == EAGAIN || errno == EWOULDBLOCK, "collect file output");
        return;
    }
}

void call_budget_and_mask_failure(const Fixture &fixture, bool disable_budget, bool disable_mask) {
    int pair[2];
    require(::socketpair(AF_UNIX, SOCK_STREAM | SOCK_NONBLOCK, 0, pair) == 0, "budget pair");
    Socket peer(pair[1]);
    int large = 1024 * 1024;
    ...
    require(interrupted.bytes_written == 0 && interrupted.error_number == 0 &&
                sendfile_injection_consumed - before == ConnectionIo::file_call_budget &&
    ...
    for (int signal = 1; signal < NSIG; ++signal)
        require(::sigismember(&original, signal) == ::sigismember(&restored, signal),
    ...
    std::cout << "M1 call_budget=" << ConnectionIo::file_call_budget
              << " real_byte_budget=" << progress.bytes_written << " guard_failure_calls=0\n";
}
...
void file_registration_failure(const Fixture &fixture, bool disabled) {
    ...
    require(failed && registration_injections == before + 1 && evidence(id).closes == 1,
...
void transport_bounds(const Fixture &fixture, bool disable_allocation) {
    ...
    require((::fcntl(replacement.fd(), F_GETFD) & FD_CLOEXEC) != 0, "file CLOEXEC");
    replacement.advance(7);
    require(evidence(moved_id).closes == 1, "region completes and releases file immediately");
    for (const auto length : {ConnectionIo::output_limit - 2, ConnectionIo::output_limit - 1,
                              ConnectionIo::output_limit}) {
        int pair[2];
        require(::socketpair(AF_UNIX, SOCK_STREAM | SOCK_NONBLOCK, 0, pair) == 0, "bounds pair");
        Socket peer(pair[1]);
        ConnectionIo io{Socket(pair[0])};
        ...
        require(rejected == (length == ConnectionIo::output_limit), "header plus file limit +/-1");
        if (rejected) {
            require(io.pending_bytes() == 0 && evidence(id).closes == 1,
                    "rejected region closes with no partial submission");
        } else {
            require(io.pending_bytes() == length + 1 && ConnectionIoTestAccess::capacity(io) < 1024,
    ...
    require(!ConnectionIo::output_fits(1, SIZE_MAX) && !ConnectionIo::output_fits(SIZE_MAX, 1),
            "overflow-safe file sum predicate");
    int pair[2];
    require(::socketpair(AF_UNIX, SOCK_STREAM | SOCK_NONBLOCK, 0, pair) == 0, "allocation pair");
    Socket peer(pair[1]);
    ConnectionIo io{Socket(pair[0])};
    ...
    require(failed && io.pending_bytes() == 0 && evidence(id).closes == 1,
...
void transport_short_writes(const Fixture &fixture) {
    int pair[2];
    require(::socketpair(AF_UNIX, SOCK_STREAM | SOCK_NONBLOCK, 0, pair) == 0, "short write pair");
    Socket peer(pair[1]);
    int size = 4096;
    require(::setsockopt(pair[0], SOL_SOCKET, SO_SNDBUF, &size, sizeof(size)) == 0,
            "small send buffer");
    ConnectionIo io{Socket(pair[0])};
    ...
    require(first.would_block && first.bytes_written > 0 && first.bytes_written < header.size() &&
                evidence(id).calls == 0,
    ...
    require(std::memcmp(wire.data() + header.size(), fixture.large.data() + 17, length) == 0,
            "nonzero offset file exact bytes");
    require(record.positive > 0 && record.short_writes > 0 && record.eagain > 0 &&
                record.bytes == length && !record.offset_error && record.closes == 1 &&
                sendfile_injection_consumed == injected_before + 1,
            "real file EAGAIN/short writes and EINTR preserve offset");
    std::cout << "M1 short header=" << first.bytes_written << " file_bytes=" << record.bytes
              << " positive=" << record.positive << " short=" << record.short_writes
              << " EAGAIN=" << record.eagain << " turns=" << turns
...
void transport_errors(const Fixture &fixture, bool disable_eof) {
    for (const auto error : {0, EIO, ENOSYS}) {
        ...
        int pair[2];
        require(::socketpair(AF_UNIX, SOCK_STREAM | SOCK_NONBLOCK, 0, pair) == 0, "error pair");
        Socket peer(pair[1]);
        registry.add(Socket(pair[0]), [&](TcpConnection &connection, auto bytes, bool) {
            auto file =
                region(fixture, error ? 32 : 1, (error || disable_eof) ? 0 : fixture.large.size());
        ...
        loop.poll_once(0);
        const auto id = latest_file();
        require(ShutdownAccess::entries(registry).empty() && evidence(id).closes == 1 &&
    ...
    std::cout << "M1 errors early_EOF=1 EIO=1 unsupported=1 no_extra_response=1\n";
}
...
void transport_fairness(const Fixture &fixture) {
    ...
    registry.add(Socket(pair[0]), [&](TcpConnection &connection, auto bytes, bool) {
        ...
        connection.set_write_complete_callback([&](TcpConnection &current) {
            ++completed;
            if (completed < 100)
                current.send_file({}, region(fixture, 1));
        });
        connection.send_file({}, region(fixture, 1));
    });
    registry.add(Socket(healthy[0]), [&](TcpConnection &connection, auto bytes, bool) {
        ++healthy_calls;
    ...
    loop.set_control_callback([&](EventLoop::Control, auto) { registry.begin_drain(true); });
    bool task = false;
...
void guarded_sigpipe(const Fixture &fixture, bool disable_peer_close) {
    ...
    if (child == 0) {
        try {
            ...
            action.sa_handler = SIG_DFL;
            ::sigemptyset(&action.sa_mask);
            require(::sigaction(SIGPIPE, &action, nullptr) == 0, "default SIGPIPE disposition");
            ...
            for (int mode = 0; mode < 3; ++mode) {
                ...
                require(result.error_number == EPIPE, "real sendfile EPIPE returned safely");
                ::pthread_sigmask(SIG_SETMASK, nullptr, &actual);
                ::sigpending(&pending);
                for (int signal = 1; signal < NSIG; ++signal)
                    require(::sigismember(&actual, signal) == ::sigismember(&saved_mask, signal),
            ...
            }
            {
                ...
                std::cerr << "TCP reset diagnostic fd=" << event.fd << " poll=" << polled
                          << " revents=" << event.revents << " errno=" << errno << std::endl;
            ...
            require(action.sa_handler == SIG_DFL, "transport did not globally ignore SIGPIPE");
    ...
    std::cout << "SIGPIPE child exit=" << (WIFEXITED(status) ? WEXITSTATUS(status) : -1)
              << " signal=" << (WIFSIGNALED(status) ? WTERMSIG(status) : 0) << std::endl;
    require(WIFEXITED(status) && WEXITSTATUS(status) == 0,
            "guarded default-SIGPIPE process survives");
    std::cout << "M1 SIGPIPE default_survival=1 blocked=preserved pending=preserved TCP_RST=1 "
                 "next_healthy=1\n";
}
} // namespace
...
struct SendfileTestAccess {
    static ConnectionRegistry &registry(TcpServer &server, std::size_t index) {
        return server.worker_count_ ? *server.registries_[index] : *server.main_registry_;
    }
    ...
    static auto progress(const TcpConnection &connection) { return connection.last_progress_; }

    static bool waiting(const TcpConnection &connection) {
        return connection.wait_since_.has_value();
    }

    static void write(TcpConnection &connection) { connection.handle_event(EPOLLOUT); }
};
} // namespace hp::net
...
std::size_t owner_capacity(ServerHarness &harness, std::size_t workers, std::size_t owner) {
    std::promise<std::size_t> answer;
    auto result = answer.get_future();
    auto observe = [&] {
        auto &entries =
            ShutdownAccess::entries(SendfileTestAccess::registry(*harness.server, owner));
        require(entries.size() == 1, "one production connection per measured owner");
        answer.set_value(SendfileTestAccess::capacity(*entries.begin()->second));
    };
    if (workers)
        require(TcpServerTestAccess::post(*harness.server, owner, [&](EventLoop &) { observe(); }),
                "capacity owner observer");
    else
        require(TcpServerTestAccess::main_post(*harness.server, observe), "capacity main observer");
    require(result.wait_for(3s) == std::future_status::ready, "capacity observer ready");
    return result.get();
}

std::string materialize_prepared(hp::http::ResponseResult prepared) {
    if (!prepared.file)
        return {reinterpret_cast<const char *>(prepared.bytes.data()), prepared.bytes.size()};
    int pair[2];
    require(::socketpair(AF_UNIX, SOCK_STREAM | SOCK_NONBLOCK, 0, pair) == 0,
...
void prepare_compatibility(const Fixture &fixture, const hp::http::StaticFileService &service) {
    Fixture::write_text(fixture.root / "empty.bin", "");
    Fixture::write_text(fixture.root / "one.bin", "X");
    Fixture::write_bytes(
        fixture.root / "kilo.bin",
        std::vector<std::byte>(fixture.large.begin(), fixture.large.begin() + 1024));
    Fixture::write_bytes(
        fixture.root / "mega.bin",
        std::vector<std::byte>(fixture.large.begin(), fixture.large.begin() + 1024 * 1024));
    for (auto path : {"/empty.bin", "/one.bin", "/large.bin", "/oversized.bin", "/escape.txt",
                      "/../sibling-secret.txt", "/assets", "/missing", "/assets/binary.png"}) {
        const auto before = file_count();
        auto prepared =
            service.prepare_response({"GET", path}, hp::http::ConnectionPolicy::keep_alive);
        ...
        const auto legacy =
            service.handle_response({"GET", path}, hp::http::ConnectionPolicy::keep_alive);
        require(actual.size() == legacy.bytes.size() &&
        ...
        for (auto id = before; id < file_count(); ++id)
            require(evidence(id).closes == 1, "prepare/compatibility close every file identity");
    }
    std::cout << "M2 prepare compatibility paths=9 zero/one/8MiB/overlimit/security byte_exact=1\n";
}

void changing_files(const Fixture &fixture, const hp::http::StaticFileService &service,
    ...
    require(evidence(old_id).inode != evidence(new_id).inode &&
                materialize_prepared(std::move(replaced)).ends_with("\r\n\r\nnew contents"),
    ...
    ConnectionRegistry registry(loop, 0);
    int pair[2];
    require(::socketpair(AF_UNIX, SOCK_STREAM | SOCK_NONBLOCK, 0, pair) == 0, "truncate pair");
    Socket peer(pair[1]);
    registry.add(Socket(pair[0]), [&](TcpConnection &connection, auto input, bool) {
    ...
    std::string wire;
    collect(peer.fd(), wire);
    require(ShutdownAccess::entries(registry).empty() && evidence(truncated_id).closes == 1 &&
                wire.starts_with("HTTP/1.1 200") && wire.ends_with("\r\n\r\n") &&
                wire.find("500") == std::string::npos,
            "truncate closes without extra error response");
    std::cout << "M2 file identity rename=old/new growth=initial_length truncate=header_only_EOF\n";
}

void production_files(const Fixture &fixture, const hp::http::StaticFileService &service) {
    for (std::size_t workers : {0U, 1U, 2U}) {
        const auto begin = file_count();
        ServerHarness harness(service, workers);
        std::size_t request_index = 0;
        for (const auto &[path, length] : std::vector<std::pair<std::string, std::size_t>>{
                 {"/kilo.bin", 1024},
                 {"/mega.bin", 1024 * 1024},
            ...
            await([&] {
                if (file_count() <= opening)
                    return false;
                const auto record = evidence(opening);
                return length == fixture.large.size() ? record.eagain > 0 : record.positive > 0;
            });
            ...
            require(received.status == 200 && received.body.size() == length &&
                        std::memcmp(received.body.data(), fixture.large.data(), length) == 0,
            ...
            client.eof();
            const auto record = evidence(opening);
            require(record.reads == 0 && record.preads == 0 && record.bytes == length &&
                        record.closes == 1 && !record.offset_error,
                    "production sendfile without read/pread fallback");
            std::cout << "M2 production workers=" << workers << " length=" << length
                      << " sendfile_bytes=" << record.bytes << " positive=" << record.positive
                      << " EAGAIN=" << record.eagain << " header=" << received.header.size()
        ...
        for (auto id = begin; id < file_count(); ++id)
            require(evidence(id).closes == 1 && evidence(id).reads == 0 && evidence(id).preads == 0,
                    "production each opened region closes exactly once");
    }
}

void file_progress(const Fixture &fixture, const hp::http::StaticFileService &service) {
    EventLoop loop;
    ConnectionRegistry registry(loop, hp::http::max_request_bytes, {150ms, 100ms});
    int pair[2];
    require(::socketpair(AF_UNIX, SOCK_STREAM | SOCK_NONBLOCK, 0, pair) == 0, "progress pair");
    Socket peer(pair[1]);
    int small = 4096;
    ...
    require(SendfileTestAccess::progress(connection) == previous &&
                connection.pending_bytes() == queued && !SendfileTestAccess::waiting(connection) &&
                providers == 1 && stats.responses == 1,
    ...
    while (!ShutdownAccess::entries(registry).empty())
        loop.poll_once(200);
    require(loop.timer_count() == 0 && evidence(id).closes == 1 && providers == 1,
            "stalled file idle closes region without pipeline suffix");
    std::cout << "M2 file_progress bytes=" << evidence(id).bytes << " refreshed_cycles=10"
              << " EAGAIN=" << evidence(id).eagain << " provider=" << providers
              << " timer=0 idle_closed=1\n";
    (void)fixture;
}
...
void unfinished_lifecycle(const hp::http::StaticFileService &service) {
    ...
    for (int cycle = 0; cycle < 100; ++cycle) {
        ...
        await([&] { return pending(harness, 2, 0) > 0 && pending(harness, 2, 1) > 0; });
        if (cycle == 99) {
            require(TcpServerTestAccess::post(
                        *harness.server, 0,
                        [](EventLoop &) { throw std::runtime_error("file fatal original"); }),
                    "file fatal accepted");
    ...
    for (auto id = files; id < file_count(); ++id)
        require(evidence(id).closes == 1 && evidence(id).bytes < hp::http::max_file_bytes,
...
int main(int argc, char **argv) {
    try {
        Fixture fixture;
        const auto option = argc == 2 ? std::string_view(argv[1]) : std::string_view{};
        ...
        require(root_errors == 0 && live_sessions == 0, "file session/service owner lifetime");
        {
            std::lock_guard lock(file_evidence_mutex);
