...
void raw_observation(const Fixture &fixture, int disabled_error) {
    ...
    require(record.positive == 1 && record.bytes == 1024 &&
                !record.offset_error && record.reads == 0 &&
                record.preads == 0 && record.injected == 3,
...
void call_budget_and_mask_failure(const Fixture &fixture, bool disable_budget,
    ...
    for (int signal = 1; signal < NSIG; ++signal)
        require(::sigismember(&original, signal) ==
                    ::sigismember(&restored, signal),
...
void transport_bounds(const Fixture &fixture, bool disable_allocation) {
    ...
    require(
        initial.fd() == -1 && initial.remaining() == 0 && moved.offset() == 3,
...
void transport_short_writes(const Fixture &fixture) {
    ...
    require(record.positive > 0 && record.short_writes > 0 &&
                record.eagain > 0 && record.bytes == length &&
                !record.offset_error && record.closes == 1 &&
...
void guarded_sigpipe(const Fixture &fixture, bool disable_peer_close) {
    ...
    if (child == 0) {
        try {
            ...
            for (int mode = 0; mode < 3; ++mode) {
                ...
                int pair[2];
                require(::socketpair(AF_UNIX, SOCK_STREAM | SOCK_NONBLOCK, 0,
                                     pair) == 0,
                        "pipe pair");
            ...
            }
            {
                ...
                require(::setsockopt(reset_peer.fd(), SOL_SOCKET, SO_LINGER,
                ...
                require(polled == 1 && (event.revents & POLLERR),
                        "TCP reset observed");
    ...
    std::cout
        << "M1 SIGPIPE default_survival=1 blocked=preserved pending=preserved TCP_RST=1 "
           "next_healthy=1\n";
}
}  // namespace
...
std::size_t owner_capacity(ServerHarness &harness, std::size_t workers,
    ...
    auto observe = [&] {
        ...
        answer.set_value(
            SendfileTestAccess::capacity(*entries.begin()->second));
    };
    if (workers)
...
void prepare_compatibility(const Fixture &fixture,
    ...
    for (auto path : {"/empty.bin", "/one.bin", "/large.bin", "/oversized.bin",
        ...
        require(actual.size() == legacy.bytes.size() &&
                    std::memcmp(actual.data(), legacy.bytes.data(),
                                actual.size()) == 0,
    ...
    std::cout
        << "M2 prepare compatibility paths=9 zero/one/8MiB/overlimit/security byte_exact=1\n";
}
...
void changing_files(const Fixture &fixture,
    ...
    std::cout
        << "M2 file identity rename=old/new growth=initial_length truncate=header_only_EOF\n";
}
...
void production_files(const Fixture &fixture,
    ...
    for (std::size_t workers : {0U, 1U, 2U}) {
        ...
        for (const auto &[path, length] :
            ...
            std::cout << "M2 production workers=" << workers
                      << " length=" << length
...
void file_progress(const Fixture &fixture,
    ...
    registry.add(Socket(pair[0]), hp::app::make_http_callback(
                                      [&](const auto &request, auto policy) {
                                          ++providers;
                                          return service.prepare_response(
                                              request, policy);
                                      },
                                      &stats));
    ...
    while (!ShutdownAccess::entries(registry).empty())
        loop.poll_once(200);
    require(
        loop.timer_count() == 0 && evidence(id).closes == 1 && providers == 1,
...
int main(int argc, char **argv) {
    try {
        ...
        {
            std::lock_guard lock(file_evidence_mutex);
            require(active_files.empty(),
                    "M0 all observed file identities closed");
