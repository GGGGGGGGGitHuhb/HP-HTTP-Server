...
struct Session {
    ...
    void advance(net::TcpConnection& connection) {
        ...
        if (stats) { ++stats->parses; if (eof) ++stats->eof_notifications; }
        ...
        if (parsed.status == http::ParseStatus::need_more && completed && parsed.request_bytes == 0) {
            phase = Phase::closing;
            connection.close_after_flush();
            return;
        }
        close = parsed.status != http::ParseStatus::complete || parsed.request.close_requested;
        const auto policy = close ? http::ConnectionPolicy::close : http::ConnectionPolicy::keep_alive;
        phase = Phase::writing;
        ...
        try {
            switch (parsed.status) {
                ...
                case http::ParseStatus::bad_request:
                    response = http::make_error_response(http::Status::bad_request, policy); break;
                case http::ParseStatus::method_not_allowed:
                    response = http::make_error_response(http::Status::method_not_allowed, policy); break;
            ...
            close = true;
            response = http::make_error_response(http::Status::internal_server_error, http::ConnectionPolicy::close);
        }
...
net::TcpConnection::MessageCallback make_http_callback(ResponseProvider provider, HttpCallbackStats* stats) {
    ...
    return [session, installed = false](net::TcpConnection& connection,
            std::span<const std::byte>, bool) mutable {
        if (!installed) {
            connection.set_write_complete_callback([session](net::TcpConnection& c) { session->drained(c); });
            installed = true;
        }
...
net::TcpServer::MessageCallbackFactory make_http_factory(const http::StaticFileService& service) {
    return [&service] { return make_http_callback([&service](const http::HttpRequest& request, http::ConnectionPolicy policy) {
