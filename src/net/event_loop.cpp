...
std::uint64_t allocate_token() {
    if (next_token == 0) throw std::overflow_error("Channel token exhausted");
    const auto token = next_token;
    next_token = token == std::numeric_limits<std::uint64_t>::max() ? 0 : token + 1;
    return token;
}
}
EventLoop::~EventLoop() noexcept { assert(channels_.empty()); }
...
void EventLoop::update_channel(Channel& c, std::uint32_t interest) {
    if (&c.loop_ != this) throw std::invalid_argument("Channel belongs to another loop");
    if (c.registered()) {
...
void EventLoop::dispatch(std::uint64_t token, std::uint32_t mask) {
    ...
    if (found == channels_.end()) { ++counters_.stale; return; }
...
void EventLoop::poll_once(int timeout_ms) {
    ...
    try {
        ...
        for (const auto& event : events) dispatch(event.data.u64, event.events);
    } catch (...) { polling_ = false; throw; }
    polling_ = false;
}
[[noreturn]] void EventLoop::loop() { while (true) poll_once(-1); }
}
