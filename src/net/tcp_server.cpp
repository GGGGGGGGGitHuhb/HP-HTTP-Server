...
std::uint16_t TcpServer::bound_port() const noexcept { return acceptor_.bound_port(); }
[[noreturn]] void TcpServer::run() { loop_.loop(); }
void TcpServer::add_connection(Socket socket) {
    if (next_identity_ == 0) throw std::overflow_error("connection identity exhausted");
    ...
    try { position->second->start(); }
    catch (...) { connections_.erase(position); throw; }
}
