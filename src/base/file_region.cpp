...
FileRegion::FileRegion(UniqueFd file, off_t offset, std::size_t length)
    ...
    if (file_.get() < 0 || offset < 0 ||
        length > static_cast<std::uintmax_t>(std::numeric_limits<off_t>::max() - offset))
        throw std::invalid_argument("invalid file region");
    const int flags = ::fcntl(file_.get(), F_GETFD);
    if (flags < 0 ||
        (!(flags & FD_CLOEXEC) && ::fcntl(file_.get(), F_SETFD, flags | FD_CLOEXEC) < 0))
        throw std::system_error(errno, std::generic_category(), "file region CLOEXEC");
}

FileRegion::FileRegion(FileRegion &&other) noexcept
    : file_(std::move(other.file_)), offset_(std::exchange(other.offset_, 0)),
